...
```
                  [ 22528 Sparse HalfKA Features (stm / nstm) ]
                                       │
                                       ▼
...
The input feature space consists of $22,528$ sparse indices ($32 \text{ King buckets} \times 11 \text{ piece channels} \times 64 \text{ target squares}$).

### Interleaved Piece Channels
Each perspective maps active pieces to 11 channels:
*   `0, 2, 4, 6, 8`: STM's own Pawn, Knight, Bishop, Rook, Queen.
*   `1, 3, 5, 7, 9`: NSTM's (opponent) Pawn, Knight, Bishop, Rook, Queen.
*   `10`: Both Kings (merged channel).

### Horizontal Mirroring
*   If the King file is $\ge 4$ (files e-h), the entire perspective (including King square and all other pieces) is horizontally mirrored (e.g., `sq ^= 7`).
*   This folds 64 possible King squares into 32, reducing feature space from 45,056 to 22,528.

### Coordinate Perspectives & Formulas

1.  **White Perspective (STM)**:
    *   King Square: $\text{king\_w\_sq}$
    *   If King is on files e-h: $\text{flip\_h} = \text{True}$; $\text{king\_mapped} = \text{king\_w\_sq} \oplus 7$.
    *   Else: $\text{flip\_h} = \text{False}$; $\text{king\_mapped} = \text{king\_w\_sq}$.
    *   $\text{bucket} = (\text{king\_mapped} \gg 3) \times 4 + (\text{king\_mapped} \ \& \ 7)$
...
2.  **Black Perspective (NSTM)**:
    *   King Square: $\text{king\_b\_sq} \oplus 56$ (Vertically flipped first)
    *   If flipped King is on files e-h: $\text{flip\_h} = \text{True}$; $\text{king\_mapped} = (\text{king\_b\_sq} \oplus 56) \oplus 7$.
...
*   **Special Moves**: Ensure castling updates both Rook and King coordinates correctly. En-passant captures must register a removal on the correct destination square of the captured pawn.
...
5.  **Final Score Scaling**:
    Convert the raw logit output to Centipawns (cp):
    $$\text{cp} = \lfloor \frac{\text{output}}{38.0} \rfloor$$
