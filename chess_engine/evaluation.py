...
def evaluate_attacks_mobility_threats(piece_bbs, occupancy_bbs):
    ...
    while temp_bb:
        ...
        if checks:
            # Square is safe if not attacked by black pawns AND (not attacked by black pieces OR defended by white)
            is_safe = (BB_SQUARES[sq] & ~black_pawn_attacks) != 0
            if is_safe:
    ...
    while temp_bb:
        ...
        if checks:
            is_safe = (BB_SQUARES[sq] & ~white_pawn_attacks) != 0
            if is_safe:
