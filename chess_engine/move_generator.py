...
from chess_engine.move import (
    encode_move, SPECIAL_MOVE_FLAG_NORMAL, SPECIAL_MOVE_FLAG_EN_PASSANT, SPECIAL_MOVE_FLAG_CASTLING,
...
def get_ls1b_index(bb):
    return count_bits((bb & -np.uint64(bb)) - np.uint64(1))
...
def init_sliders_attacks():
    ...
    for sq in range(64):
        ...
        for i in range(1 << relevant_bits_count):
            ...
            magic_index = (occ * BISHOP_MAGIC_NUMBERS[sq]) >> np.uint64(64 - BISHOP_RELEVANT_BITS[sq])
        ...
        for i in range(1 << relevant_bits_count):
            occ = set_occupancy(i, relevant_bits_count, attack_mask)
            magic_index = (occ * ROOK_MAGIC_NUMBERS[sq]) >> np.uint64(64 - ROOK_RELEVANT_BITS[sq])
...
def _precompute_leaper_attacks():
    for sq in range(64):
        ...
        KING_ATTACKS[sq] = (((bb << 1) | (bb << 9) | (bb >> 7)) & NOT_H_FILE) | (((bb >> 1) | (bb >> 9) | (bb << 7)) & NOT_A_FILE) | (bb << 8) | (bb >> 8)
...
def is_square_attacked(sq, attacker_side, board_state):
    ...
    if attacker_side == WHITE:
        # A square is attacked by a WHITE pawn if a pawn exists on the squares SOUTH of it.
    ...
    else: # attacker_side is BLACK
        # A square is attacked by a BLACK pawn if a pawn exists on the squares NORTH of it.
        pawn_attacks = (((BB_SQUARES[sq] & NOT_H_FILE) << 7) | ((BB_SQUARES[sq] & NOT_A_FILE) << 9))
...
def generate_pseudo_legal_moves(board_state):
    ...
    if side_to_move == WHITE:
        ...
        double_pushes = ((single_pushes & RANK_3) << 8) & empty_squares & RANK_4
        ...
        while pushes:
            ...
            from_sq = to_sq - 8
            if to_sq >= 56:
                for p_type in [PROMO_QUEEN, PROMO_ROOK, PROMO_BISHOP, PROMO_KNIGHT]: moves[move_count] = encode_move(from_sq, to_sq, p_type, 0); move_count += 1
            else:
...
def generate_legal_moves(board_state):
    ...
    legal_moves = np.zeros(256, dtype=np.uint16)
    legal_move_count = 0
    ...
    original_game_state = (board_state[17], board_state[15], np.int8(board_state[16]), 0, np.uint64(0))
    ...
    for i in range(len(pseudo_legal_moves)):
        ...
        # using the CORRECT occupancy bitboards AND en passant square returned by make_move.
