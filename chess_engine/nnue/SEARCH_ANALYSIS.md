# NNUE 搜尋演算法分析：理念、意義、細節與方法論

本文件對 NNUE 模組中實現的搜尋演算法（[search.py](search.py)）及相關模組（如 [constants.py](constants.py)、[transposition_table.py](transposition_table.py)）進行了全面的分析。它闡明了底層邏輯（理念）、參數的意義、具體的實現選擇以及整體的算法方法論。
...
**核心檔案：**
*   [search.py](search.py)：核心搜尋邏輯（迭代加深、PVS、靜態搜尋）。
*   [constants.py](constants.py)：調優參數、剪枝邊界與權重。
*   [transposition_table.py](transposition_table.py)：置換表（哈希表）的實現。
...
## 8. 動態搜尋開關與 UCI 整合 (Dynamic Search Context & UCI Integration)

為了支援搜尋演算法的動態調參及自動化測試，NNUE 版本的搜尋模組（`chess_engine/nnue/search.py`）引入了動態布林開關整合。

*   **理念**：傳統 JIT 編譯中，採用全域常數（如 `constants.py` 中的 `ENABLE_*`）作為剪枝開關會導致修改時需重新編譯 JIT 核心。藉由將這些剪枝開關（如 NMP, RFP, LMR, ProbCut 等）作為屬性整合進 `@jitclass` 的 `SearchContext`，可在不重新編譯的情況下由外部動態控制。
*   **運作機制**：當透過 UCI `setoption` 指令設定搜尋選項時，`main_nn.py` 會將其記錄於配置中，並在啟動搜尋（`go` 指令）時寫入到 `SearchContext` 實例的對應欄位，從而在維持極限 JIT 效能的同時支援執行期即時調參。
