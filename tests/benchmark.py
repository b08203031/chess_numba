...
# Create SearchContext needed for search / ?µå»º?œå??€?€?„ä?ä¸‹æ?
...
def run_benchmark_for_fen(fen, depth, name):
    ...
    ?ºçµ¦å®šç? FEN å­—ä¸²?‹è??°å›ºå®šæ·±åº¦ç??œå??ºæ?æ¸¬è©¦??
    """
    # No time limit, run to completion for the given depth / ?¡æ??“é??¶ï??‹è??°å??çµ¦å®šæ·±åº?
    ...
    # Clear TT before search / ?œå??æ??¤ç½®?›è¡¨
    clear_transposition_table(transposition_table)
    
    # Reset search context stats / ?ç½®?œå?ä¸Šä??‡çµ±è¨ˆæ•¸??
...
def run_all_benchmarks():
    """Runs all benchmark tests. / ?‹è??€?‰åŸºæº–æ¸¬è©¦ã€?""
