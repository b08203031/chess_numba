        ...
        try:
            ...
            if command == "uci":
                ...
                try:
                    ...
                    opt_val = " ".join(tokens[value_idx+1:])
                    if opt_name == "OwnBook":
                        own_book = opt_val.lower() == "true"
                        print(f"info string OwnBook set to {own_book}", flush=True)
                except ValueError:
                    pass
                ...
                # Only use book if own_book option is True and halfmove clock is low (early game)
                if own_book and board_state[2][3] < 20:
